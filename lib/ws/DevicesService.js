...
function DevicesService(app) {
    ...
    return function() {
        ...
        app.get('/available', function(req,res) {
            ...
            var devicesToPing = req.query.phone_numbers

            if(devicesToPing) {
                devicesHelper.ping_devices(devicesToPing, function(err, ping_id) {
                    if(err) return res.send(err)
                    console.log("Ping id : " + ping_id)
                    res.send(200, ping_id)
                })
            } else {
                devicesHelper.ping_all_devices(function(err, ping_id) {
                    if(err) return res.send(err)
